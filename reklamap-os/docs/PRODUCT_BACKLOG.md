...
| B-7 | Stawka przy umowie długoterminowej: opcjonalne pole `price_long_term` (zł/mc przy 12 mies.) obok `price` — „od X zł/mc przy umowie rocznej" na karcie, w liście, PDF i porównywarce | cała baza w wyświetlaniu; dziś 5 operatorów / 69 ogłoszeń ma stawki okresowe schowane w opisie | 2 | 80% | ~0.5–1 tyg | **~300** | TODO — sugestia operatora PRS 2026-09-14; rozszerzenie B-5 |
...
### B-7 — Stawka przy umowie długoterminowej (`price_long_term`)
- **SYGNAŁ:** operator PRS Reklama Przemyśl (2026-09-14, po imporcie 64 tablic) — „obok ceny za miesiąc warto pokazać cenę przy wynajmie na 12 miesięcy; jako klient chciałbym to widzieć od razu przy ofercie". To piąty operator z cennikiem schodkowym (1/3/6/12 mies.): PRS 800→500 zł, dwustronne 1000→700 zł. Na prod 69 ogłoszeń ma stawkę roczną tylko w treści opisu — niewidoczną w liście, filtrach i porównywarce. AMS i Bauer też publikują cenniki zależne od okresu (CLAUDE.md, 2026-08-21).
- **PROPONOWANA FUNKCJA:** jedno opcjonalne pole `price_long_term` (decimal, zł/mc netto przy umowie 12 mies.). `price` zostaje stawką bazową (1 mies.) — jedyną filtrowalną i sortowalną, jak w B-5. Wyświetlanie: karta w liście „800 zł/mc · od 500 zł/mc przy umowie rocznej", strona ogłoszenia, PDF, porównywarka (wiersz „stawka roczna"). Formularz dodawania: pole w kroku „opcjonalny" (B-2), z podpowiedzią „jeśli dajesz rabat za dłuższą umowę". Import: skrypty PRS / BrokersMedia / Big Group mają te dane w źródłach — do uzupełnienia update'em w miejscu.
- **CZEGO NIE ROBIĆ:** pełnej tabeli progów (3/6/12) w modelu — 90 % wartości daje jedna liczba; pełne progi zostają w opisie. Nie sortować/filtrować po `price_long_term` (mieszałoby porównanie z ogłoszeniami bez rabatu).
- **RICE:** Reach cała baza w wyświetlaniu · Impact 2 (klient szukający kampanii rocznej widzi realną cenę; operator ma argument bez dopisywania w opisie) · Confidence 80 % (prośba wprost od operatora + wzorzec w cennikach AMS/Bauer) · Effort ~0.5–1 tyg (migracja + model + walidacja + types + 2 formularze + 4 widoki + PDF + 3 skrypty importu) → **~300**. Najtaniej razem z B-5 (te same pliki).
