---
title: "Reklama w transporcie publicznym — ceny i formaty 2026"
slug: "reklama-w-transporcie-publicznym"
category: poradniki
meta_description: "Reklama w transporcie publicznym 2026: cenniki MPK Poznań, Kraków, Łódź, Wrocław i MZK Bydgoszcz. Sprawdź, co obejmuje cena →"
image_alt: "Autobus miejski z pełnym oklejeniem reklamowym jadący przez centrum miasta"
image_prompt: "A city bus with a full wrap advertisement driving through an urban street, people on sidewalk, golden hour light, realistic professional photography, no text on image"
keywords:
  - reklama w transporcie publicznym
  - reklama na autobusie
  - reklama w komunikacji miejskiej
  - ile kosztuje reklama na autobusie
word_count: ~2100
published_at: "2026-03-25 08:00:00"
status: draft
---

<!-- Angle: Praktyczny przewodnik — jakie formaty reklamy w komunikacji miejskiej wybrać i ile kosztują naprawdę w Polsce 2026 -->

# Reklama w transporcie publicznym — ceny i formaty 2026

Reklama modułowa na boku autobusu zaczyna się u operatorów miejskich od **ok. 1 000 zł miesięcznie** (MPK Poznań), a całopojazdowa ekspozycja na autobusie solo to 1 250–1 500 zł/mies. (MPK Wrocław). Pakiety agencyjne obejmujące flotę i produkcję kosztują wielokrotnie więcej. To jeden z niewielu formatów outdoor, gdzie cena od razu obejmuje produkcję materiałów. Poniżej kompletny cennik i praktyczny przewodnik jak wybrać format, który działa na Twój cel.

## Jakie formaty reklamy oferuje transport publiczny?

Reklama w komunikacji miejskiej dzieli się na dwie kategorie: zewnętrzną (widoczna dla przechodniów i kierowców) i wewnętrzną (dla pasażerów w środku pojazdu).

**Zewnętrzna:**

- **Full cover** — pełne oklejenie autobusu lub tramwaju. Pojazd staje się mobilnym billboardem jeżdżącym przez całe miasto przez cały dzień.
- **Reklama modułowa boczna** — częściowe oklejenie jednej lub obu burt pojazdu. Niższy koszt wejścia, nadal wysoka widoczność dla pieszych.
- **Całopojazdowa tramwaj** — pełne oklejenie tramwaju; w miastach z gęstą siecią tramwajową (Kraków, Wrocław, Poznań) bardzo efektowny format.

**Wewnętrzna:**

- **Ramki reklamowe** — plakaty w uchwytach ponad oknami lub przy drzwiach. Pasażer, czekając na swój przystanek, naturalnie kieruje wzrok na ściany pojazdu.
- **Ekrany LCD** — w nowoczesnym taborze; dynamiczny przekaz z możliwością animacji i QR kodów.

Kluczowa przewaga formatu wewnętrznego: pasażer jest "uwięziony" w pojeździe i ma czas na przeczytanie dłuższego komunikatu. To unikalny kontekst, niedostępny dla billboardu czy ekranu LED.

## Ile kosztuje reklama w komunikacji miejskiej? Cennik 2026

Uwaga na dwie różne logiki cenowe: **operatorzy miejscy podają samą ekspozycję** (MPK Poznań i MPK Wrocław wprost zastrzegają w regulaminach, że projekt, druk, montaż, demontaż i przywrócenie barw pojazdu to osobny koszt zamawiającego), a **pośrednicy sprzedają pakiety** obejmujące produkcję i wiele pojazdów naraz. Poniższe pozycje pochodzą z obu źródeł — sprawdzaj, co dokładnie zawiera oferta:

| Format | Cena netto/mc |
|--------|--------------|
| Reklama modułowa boczna — autobus (MPK Poznań) | 1 000 zł/mies. |
| Reklama modułowa boczna — tramwaj Moderus Beta (MPK Poznań) | 3 000 zł/mies. |
| Ramki wewnętrzne u operatora (MPK Poznań) | 18–23 zł/szt./tydzień |
| Całopojazdowa — autobus solo (MPK Wrocław) | 1 250–1 500 zł/mies. |
| Reklama na burcie tramwaju Moderus Beta (Poznań) | 7 800 zł |
| Całopojazdowa — tramwaj (Poznań) | 10 300–12 500 zł |
| Busboard — bok autobusu (MPK Wrocław) | 833–1 000 zł/mies. |
| Fullback — tył autobusu (MPK Łódź) | 1 600 zł/mies. |
| Cały wagon tramwaju bez szyb (MPK Łódź) | 8 500 zł/mies. |

**Częsty błąd w budżetowaniu:** przy ofertach operatorów miejskich cena NIE obejmuje produkcji — MPK Poznań i MPK Wrocław wprost przenoszą koszt projektu, druku, montażu i demontażu na zamawiającego. Skalę tej różnicy najlepiej widać w Krakowie, bo MPK rozlicza obie pozycje osobno i obie publikuje: ekspozycja na tramwaju E1 to **4 244 zł miesięcznie**, a produkcja — druk, nałożenie i późniejsze usunięcie z przywróceniem barw firmowych — **13 200 zł**. Miesięczna kampania kosztuje więc realnie **17 444 zł, z czego 76% to produkcja, a nie ekspozycja**. Przy umowie rocznej proporcja się odwraca, bo produkcję płacisz raz.

Dlatego pierwsze pytanie do sprzedawcy nie brzmi „ile kosztuje", tylko **„czy ta cena obejmuje produkcję"**. Bez odpowiedzi na nie porównywanie ofert między miastami i między pośrednikami nie ma sensu.

Szczegółowe porównanie kosztów: [Ile kosztuje reklama outdoor?](/blog/poradniki/ile-kosztuje-reklama-outdoor)

Przeglądaj dostępne nośniki w transporcie w swojej okolicy → [reklamap.pl/powierzchnie-reklamowe](/powierzchnie-reklamowe)

## Cennik komunikacji miejskiej w pięciu miastach

Poniższe stawki pochodzą z **oficjalnych cenników operatorów**, nie od pośredników. Wszystkie są netto. Cenniki się starzeją — przy każdym podano datę obowiązywania.

### Zanim porównasz miasta: sprawdź, czy w cenie jest produkcja

To pytanie decyduje o budżecie bardziej niż sama stawka. Weźmy Kraków: ekspozycja reklamy na tramwaju E1 kosztuje **4 244 zł miesięcznie**, ale produkcja — wydruk, oklejenie pojazdu i późniejsze zdjęcie z przywróceniem barw firmowych — to osobne **13 200 zł**.

Przy kampanii miesięcznej **produkcja jest ponad trzykrotnie droższa od ekspozycji**: realny koszt to 17 444 zł, z czego 76% idzie na oklejenie, nie na wynajem. Przy kampanii rocznej proporcja się odwraca — produkcja stanowi wtedy 54% kosztu ekspozycji, bo pojazd oklejasz raz.

**Praktyczny wniosek:** reklama całopojazdowa w komunikacji miejskiej opłaca się przy kampaniach długich. Przy jednomiesięcznej płacisz głównie za folię.

| Miasto | Cały tramwaj, 1 miesiąc | Produkcja |
|---|---:|---|
| Wrocław (Škoda 19T) | 4 000 zł | osobno, poza cennikiem |
| Kraków (E1) | 4 244 zł | osobno, 13 200 zł |
| Łódź (Cityrunner / NF6D) | 8 500 zł | do ustalenia w umowie |
| Poznań (Moderus Beta) | 10 300 zł | po stronie zamawiającego |

Rozpiętość jest 2,6-krotna — ale liczba z nagłówka niewiele znaczy, dopóki nie wiesz, co obejmuje.

### Poznań — MPK

Najdroższy tramwaj z całej piątki, ale też najbogatsza oferta modułowa.

| Format | Cena netto/mies. |
|---|---:|
| Całopojazdowa, tramwaj | 10 300 zł (bez frontów) – 12 500 zł (z frontami) |
| Burta tramwaju Moderus Beta | 7 800 zł |
| Reklama modułowa, tramwaj Moderus Beta | 3 000 zł |
| Reklama modułowa, autobus | 1 000 zł |
| Ramki wewnętrzne | 18–23 zł/szt./tydzień |

Regulamin MPK Poznań przenosi na zamawiającego koszty projektu, przygotowania powierzchni, naniesienia, odnowienia i demontażu. Kontekst rynku lokalnego opisaliśmy w tekście o [reklamie outdoor w Poznaniu](/blog/lokalizacje/reklama-outdoor-poznan).

### Gdańsk — GAiT

Gdańskie Autobusy i Tramwaje **nie publikują cennika**. Oferta obejmuje ramki, plakaty, oklejanie pojazdów oraz formaty fullback i halfback, ale każda wycena powstaje indywidualnie — zależnie od liczby pojazdów, zakresu prac i długości kampanii.

Dla planującego budżet to istotna różnica proceduralna: w Poznaniu, Krakowie, Łodzi, Wrocławiu i Bydgoszczy policzysz kampanię samodzielnie w kilka minut. W Gdańsku każdy wariant wymaga kontaktu i czekania na ofertę.

### Kraków — MPK (cennik od 1 grudnia 2025)

| Pojazd | Ekspozycja 1 mies. | Ekspozycja 12 mies. | Produkcja |
|---|---:|---:|---:|
| Tramwaj E1 | 4 244 zł | 24 246 zł | 13 200 zł |
| Tramwaj E1+C3 | 7 516 zł | 42 430 zł | 23 100 zł |
| Tramwaj NGT6 / EU8N / GT8N | 12 365 zł | 65 462 zł | 21 800 zł |

Dwie rzeczy, które trzeba wiedzieć przed podpisaniem umowy: **pojazdy nie są przypisane do konkretnych linii**, a MPK gwarantuje minimum **21 dni jazdy w miesiącu**. Jeśli zależy Ci na konkretnej trasie przez centrum, komunikacja miejska nie jest właściwym narzędziem — dokładniej celuje [billboard albo citylight](/blog/poradniki/ile-kosztuje-reklama-outdoor). Lokalne uwarunkowania prawne opisaliśmy w tekście o [reklamie outdoor w Krakowie](/blog/lokalizacje/reklama-outdoor-krakow).

### Łódź — MPK (cennik od 1 marca 2025)

Jedyne miasto z piątki, które publikuje pełną siatkę cen zależnych od długości ekspozycji — im dłużej, tym taniej za miesiąc.

| Format | Cena netto |
|---|---:|
| Tramwaj, cały wagon bez szyb (Cityrunner / NF6D) | 8 500 zł do 1 mies. · 12 000 zł / 3 mies. · 18 000 zł / 6 mies. · 23 000 zł / 12 mies. |
| Tramwaj NF6D, 2 moduły na burcie | 4 000 zł / 3 mies. · 6 000 zł / 6 mies. · 8 000 zł / 12 mies. |
| Tramwaj NF6D, 8 modułów | 10 000 zł / 3 mies. · 15 000 zł / 6 mies. · 18 000 zł / 12 mies. |
| Autobus — fullback (tył) | 1 600 zł/mies. (1 200 zł za pół miesiąca) |
| Autobus — tylna szyba lub klapa | 1 350 zł/mies. |
| Ramka A3 (autobusy i tramwaje) | 50 zł/mies. |
| Plakat na oparciu fotela (tylko tramwaje) | 40 zł/mies. |

Moduł burtowy ma 2100×600 mm. Flota dostępna w kampaniach to **250 autobusów** (bez elektrycznych) i **94 tramwaje** (bez Moderusów). Wymiana grafiki w trakcie kampanii kosztuje dodatkowo: 350 zł netto za dzień hali napraw przy tramwajach, 120 zł za pojazd przy autobusach. Kontekst rynku: [reklama outdoor w Łodzi](/blog/lokalizacje/reklama-outdoor-lodz).

### Wrocław — MPK

| Pojazd | Ekspozycja 1 mies. | Ekspozycja 12 mies. |
|---|---:|---:|
| Tramwaj Moderus Beta MF 19 AC | 4 500 zł | 45 000 zł |
| Tramwaj Pesa TWIST 2010NW | 4 500 zł | 45 000 zł |
| Tramwaj Škoda 19T | 4 000 zł | 40 000 zł |
| Autobus solo, całopojazdowa | 1 500 zł | 15 000 zł |
| Busboard (bok autobusu) | 833–1 000 zł | — |

Opłata z góry za rok jest o około 17% korzystniejsza od dwunastu opłat miesięcznych. **Minimalny czas ekspozycji przy rozliczeniu miesięcznym to 3 miesiące.** Cennik nie obejmuje projektu graficznego, druku, montażu ani demontażu z przywróceniem barw MPK.

### Bydgoszcz — MZK

| Format | Cena netto |
|---|---:|
| Fullback (tył autobusu: szyba i powierzchnia lakierowana) | 1 000 zł/mies. |
| Powierzchnia na obiektach MZK | 70 zł/m²/mies. |
| Ekran LCD w tramwaju Swing 122 NaB | 1 500 zł (10 s) · 2 000 zł (15 s) · 3 000 zł (30 s) za 7 dni |
| Ramka A3 w tramwaju | 20–25 zł/szt. |

Bydgoszcz jako jedyna z piątki ma w publicznym cenniku **reklamę na ekranach LCD w tramwajach** rozliczaną tygodniowo — to najniższy próg wejścia w całym zestawieniu.

## Kto jest odbiorcą reklamy w transporcie?

Transport publiczny dociera do wyraźnie określonych grup:

- **Uczniowie i studenci** — masowi użytkownicy komunikacji, aktywni konsumenci nowych produktów
- **Pracownicy biurowi** — dojazdy do pracy w godzinach szczytu (7–9, 15–18)
- **Seniorzy** — wysoka lojalność wobec komunikacji miejskiej, szczególnie w dużych miastach
- **Turyści** — zwłaszcza w miastach z rozbudowaną siecią tramwajową (Kraków, Wrocław, Poznań)

Reklama wewnętrzna dociera przede wszystkim do pasażerów. Reklama zewnętrzna — do pieszych, rowerzystów i kierowców mijających pojazd.

## Jak zaplanować kampanię w transporcie?

**Wybierz linie, nie "całe miasto".** Zamiast kupować ekspozycję w całej sieci, wskaż konkretne linie przebiegające przez Twoją grupę docelową. Linia łącząca centrum z osiedlami mieszkaniowymi to inny odbiorca niż linia przez strefy przemysłowe.

**Połącz zewnętrzne z wewnętrznym.** Reklama zewnętrzna buduje świadomość marki u przechodniów i kierowców. Reklama wewnętrzna skłania pasażera do działania — ma czas przeczytać szczegóły i zeskanować QR kod. Oba formaty na jednej linii działają synergicznie.

**Uwzględnij minimalny czas kampanii.** Operatorzy często wymagają minimalnego okresu wynajmu. MPK Wrocław wymaga minimum 3 miesięcy przy rozliczeniu miesięcznym, a korzystniejszą stawkę daje opłata z góry za 12 miesięcy. Sprawdź warunki przed podpisaniem umowy — krótsze okresy bywają droższe proporcjonalnie.

**Korzystaj z publicznych cenników.** MPK Poznań i MPK Wrocław mają publicznie dostępne cenniki na swoich stronach. To dobry punkt startowy do weryfikacji ofert operatorów komercyjnych.

## Reklama w transporcie vs billboard — kiedy co wybrać?

| Kryterium | Reklama w transporcie | Billboard |
|---|---|---|
| Cena wejścia | od 1 350 zł/mc (z drukiem) | od 500 zł/mc (bez druku) |
| Czas kontaktu z przekazem | Długi (pasażerowie, wielokrotne przejazdy) | Krótki (2–3 sek. dla kierowców) |
| Zasięg geograficzny | Trasa pojazdu | Stała lokalizacja |
| Elastyczność | Zależna od tras operatora | Pełna swoboda wyboru lokalizacji |
| Przekaz | Może być dłuższy, bardziej szczegółowy | Musi być prosty i natychmiastowy |

Transport sprawdza się lepiej przy komunikatach wymagających czasu na przeczytanie — szczegóły oferty, QR kod, numery telefonów. Billboard to format budowania zasięgu masowego z prostym, błyskawicznym przekazem.

Jak wybrać odpowiedni format do celu kampanii: [Jak wybrać powierzchnię reklamową?](/blog/poradniki/jak-wybrac-powierzchnie-reklamowa)

## Najczęściej zadawane pytania

**Ile kosztuje reklama całopojazdowa na autobusie?**
Sama dzierżawa powierzchni to ok. 1 500 zł miesięcznie za autobus solo i 2 200 zł za przegubowy (MPK Wrocław), przy umowie rocznej odpowiednio 15 000 i 22 000 zł za cały rok. **Do tego dochodzi produkcja**, której cenniki operatorów nie obejmują: projekt, druk, oklejenie i późniejszy demontaż z przywróceniem barw firmowych. W Krakowie, gdzie MPK wycenia ją osobno, produkcja na tramwaju kosztuje 13 200–23 100 zł, a sam demontaż reklamy z tramwaju we Wrocławiu to 4 900–7 000 zł. Realny budżet kampanii jest więc zwykle kilkukrotnie wyższy od stawki z nagłówka cennika.

**Jak długo trwa realizacja kampanii w transporcie?**
Od złożenia zamówienia do pojawienia się reklamy na pojeździe zazwyczaj 2–3 tygodnie — czas na projekt graficzny, druk i oklejenie. Planuj z wyprzedzeniem, szczególnie przy kampaniach sezonowych.

**Czy można wynająć reklamy tylko na wybranych liniach?**
Tak, operatorzy oferują zazwyczaj możliwość wskazania konkretnych linii lub pojazdów. Precyzyjny dobór tras zwiększa skuteczność kampanii — płacisz za dotarcie do swojej grupy, nie do wszystkich pasażerów w mieście.

---

**Ile kosztuje reklama na tramwaju w Krakowie?**
Ekspozycja na tramwaju E1 to 4 244 zł netto miesięcznie, a przy umowie rocznej 24 246 zł. Do tego dochodzi produkcja — 13 200 zł za wydruk, oklejenie i późniejsze przywrócenie barw. Większe składy są droższe: E1+C3 to 7 516 zł miesięcznie, a tramwaje przegubowe NGT6, EU8N i GT8N — 12 365 zł.

**Czy cena reklamy w komunikacji miejskiej obejmuje oklejenie pojazdu?**
Zwykle nie, i to jest najczęstszy błąd w budżetowaniu. MPK Poznań i MPK Wrocław wprost przenoszą koszt projektu, druku, montażu i demontażu na zamawiającego. Kraków wykazuje produkcję w osobnej pozycji cennika. Przy kampanii jednomiesięcznej produkcja potrafi kosztować trzy razy więcej niż sama ekspozycja — dlatego reklama całopojazdowa opłaca się przy kampaniach długich.

**W którym mieście reklama w transporcie jest najtańsza?**
Zależy od formatu. Za cały tramwaj najmniej zapłacisz we Wrocławiu (4 000 zł/mies. za Škodę 19T) i w Krakowie (4 244 zł za E1), najwięcej w Poznaniu (10 300 zł). Ale najniższy próg wejścia w całym zestawieniu ma Bydgoszcz — reklama na ekranie LCD w tramwaju kosztuje tam od 1 500 zł za siedem dni.

**Powiązane artykuły:** [Ile kosztuje reklama outdoor?](/blog/poradniki/ile-kosztuje-reklama-outdoor) · [Jak wybrać powierzchnię reklamową?](/blog/poradniki/jak-wybrac-powierzchnie-reklamowa) · [Reklama outdoor w Warszawie](/blog/lokalizacje/reklama-outdoor-warszawa)

Szukasz nośników reklamowych w transporcie w swoim mieście? Przeglądaj dostępne oferty bezpośrednio od operatorów → [reklamap.pl/powierzchnie-reklamowe](/powierzchnie-reklamowe)
