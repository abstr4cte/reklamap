...
|---|---|---|---|---|
| 1 | **Filtr województwa zwraca 0 dla 13 z 16 województw.** `AdvertisementController.php:257` robi exact match `where('region', …)`; front wysyła ASCII-id z `polishLocations.json` (`dolnoslaskie`), a baza ma `śląskie` LUB `województwo śląskie` LUB NULL (**480/827 = 58% pustych**) | Pomiar na prod API wszystkich 16 id: działają tylko `slaskie` 135, `lubelskie` 32, `zachodniopomorskie` 80. `dolnoslaskie` → **0 przy ~407 nośnikach** realnie w regionie (bbox lat/lng). Filtr sięga **261/827 (31,6%)** | **Patch gotowy i przetestowany** (`fix-region.patch`, backend 4/4, front 143/143) — fold jak w `city_strict` + prefiks „województwo ". Podnosi zasięg do 42%; reszta wymaga `region:canonize` (dry-run domyślnie, UPDATE w miejscu, bez ruszania `updated_at`) | **KRYTYCZNE** |
