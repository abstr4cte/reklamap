...
**Metoda:** 183 inspekcje URL Inspection API · 218 pobrań jako Googlebot + 54 testy curl × 3 UA · **12 przebiegów Lighthouse 13.4.1 na żywym prodzie** + sonda `PerformanceObserver` · Bing API (13 metod) · prod API 827 ogłoszeń. Sześć krytycznych znalezisk przeszło adwersaryjną weryfikację (3 obalone).

### 🔴 DO NAPRAWY — potwierdzone pomiarem

| # | Znalezisko | Dowód | Fix | Waga |
|---|---|---|---|---|
| 1 | ✅ **NAPRAWIONE I WDROŻONE 2026-07-26** (commit `09fac15`, backend + front). Pomiar na prodzie po deployu: **261 → 347 ogłoszeń w zasięgu filtra**; 9 województw wyszło z zera (`dolnoslaskie` 0→10, `malopolskie` 0→9, `wielkopolskie` 0→9, `warminsko-mazurskie` 0→4, `lodzkie` 0→3, `lubuskie` 0→2, `podkarpackie`/`podlaskie`/`swietokrzyskie` 0→1), 4 zyskały na foldzie prefiksu (`mazowieckie` 14→34, `slaskie` 135→149, `zachodniopomorskie` 80→90, `lubelskie` 32→34). Pozostałe 3 (`kujawsko-pomorskie`, `opolskie`, `pomorskie`) zwracają 0, bo tamtejsze nośniki mają **puste** `region` — to część z 480 rekordów do uzupełnienia osobno, nie usterka kodu. Front: `normalizeRegionMatch` potwierdzony w produkcyjnym bundlu. ~~Opis pierwotny:~~ **Filtr województwa zwracał 0 dla 13 z 16 województw.** `AdvertisementController.php:257` robi exact match `where('region', …)`; front wysyła ASCII-id z `polishLocations.json` (`dolnoslaskie`), a baza ma `śląskie` LUB `województwo śląskie` LUB NULL (**480/827 = 58% pustych**) | Pomiar na prod API wszystkich 16 id: działają tylko `slaskie` 135, `lubelskie` 32, `zachodniopomorskie` 80. `dolnoslaskie` → **0 przy ~407 nośnikach** realnie w regionie (bbox lat/lng). Filtr sięga **261/827 (31,6%)** | **Patch gotowy i przetestowany** (`fix-region.patch`, backend 4/4, front 143/143) — fold jak w `city_strict` + prefiks „województwo ". Podnosi zasięg do 42%; reszta wymaga `region:canonize` (dry-run domyślnie, UPDATE w miejscu, bez ruszania `updated_at`) | **KRYTYCZNE** |
| 2 | **Sitemapa frontu rozjeżdża się z generatorem — publikacja bez deployu = `noindex`.** `BlogPost::saved` → `Cache::forget('sitemap_xml')` (`BlogPost.php:39`) wpuszcza URL natychmiast, ale prerender powstaje wyłącznie w `deploy.sh` | front 983 `<loc>` vs api 987. 4 URL-e (w tym **2 opublikowane, zrecenzowane artykuły**) serwują `noindex, follow` + szkielet 7 929 B z tytułem strony głównej, od 12 dni | Deploy zamyka doraźnie. Systemowo: **check driftu w `seo:tripwire`** — dziś czyta tylko sitemapę frontu (`SeoTripwire.php:74`), więc z definicji driftu nie widzi | **WYSOKIE** |
| 3 | **12 URL-i bloga zamrożonych w werdykcie „Błąd serwera (5xx)" z 2026-05-15 — a walidacja GSC UTKNĘŁA NA 69 DNI** | `pageFetchState=SERVER_ERROR`, `verdict=NEUTRAL`, `lastCrawlTime=2026-05-15` dla wszystkich 12 — **potwierdzone URL Inspection API 2026-07-25, już PO deployu**. **0 wyświetleń** vs 1 532 u 6 zdrowych. `lastmod` nie jest przyczyną (11 z 12 ma ten sam co zdrowy `billboard-reklama`).<br>**🔴 PRZYCZYNA ŹRÓDŁOWA USTALONA 2026-07-25 (kod + git):** to **nie** był brak prerendera — crawl 15.05 wyprzedził wygaśnięcie prerender.io (18.05) o 3 dni. Mechanizm: `.htaccess` w commicie `90598cd` (12.05, linie 62–65) kierował **Googlebota** na `prerender-proxy.php`; gdy trial prerender.io zaczął odrzucać żądania, proxy oddawało **`http_response_code(503)`** (`prerender-proxy.php:68`, komentarz w linii 34 wprost: „5xx od prerender.io, timeout curl → httpCode 0"). Googlebot dostał 503 na **cały batch naraz** — stąd identyczne `lastCrawlTime=2026-05-15` u wszystkich 12. Blok zakomentowano dopiero 18.05 (`a289d28`), tego samego dnia ruszyła walidacja.
**Dlaczego Google nie wraca 71 dni:** 503 ≠ 404. Czterysta cztery = „usuń z indeksu"; **503 = „serwer ma kłopot, wrócę później"** → Googlebot wchodzi w **wykładniczy backoff** (odstępy rosną do tygodni), a URL zostaje w kolejce z priorytetem bliskim zeru. Backoff nakłada się na dwa inne ustalenia audytu: **zero domen linkujących** (minimalny crawl budget) i **900 URL-i w „wykryte, niezindeksowane"** (konkurencja o ten budżet). **Dowód siły backoffu: zmiana `lastmod` na 2026-07-13 NIE sprowadziła Googlebota** — świeży `lastmod` normalnie wystarcza.
**Walidacja GSC nie zleca crawlowania** — tylko obserwuje, czy Google sam zajrzy. Skoro nie zagląda, wisi bezterminowo. **Dlatego w UI nie ma przycisku „Sprawdź poprawkę"** — GSC uważa, że walidacja już trwa. **„Poproś o zindeksowanie" to jedyny mechanizm omijający backoff** (kolejka priorytetowa); nie zastąpi go ani sitemap, ani `lastmod`, ani API. | **Nie czekać na tę walidację.** Wymusić crawl: **Sprawdź URL → Poproś o zindeksowanie** dla każdego z 12 (limit ~10–12/dzień, mieści się w jednym podejściu). Kolejność wg popytu: telebim (LED 1 260) → transport (994) → warszawa (696) → reklama-zewnetrzna (675) → jak-wybrac → wroclaw (275) → reklama-na-samochodzie (248) → totem (154) → baner-cena (130) → tablica (61) → 2 kategorie.<br>Opcjonalnie: „Sprawdź szczegóły" → **ZATRZYMAJ WERYFIKACJĘ** → przycisk „Sprawdź poprawkę" wraca i można ją uruchomić na aktualnym stanie. | **WYSOKIE** |
| 4 | **CLS 0,809–0,963 na 10 z 12 przebiegów** (próg „zły" = 0,25). `app.mount()` (`main.ts:43`) nie czeka na `router.isReady()`, trasy leniwe (`router.ts:31,59,108`) → prerenderowana treść **znika** (24 kafle → 0, HTML 197 779 → 86 116 zn.) i wraca po **0,55 s do 13,7 s** | Zmierzone klatka po klatce. Kontrola negatywna: **home ma CLS 0,000**, bo `HomePage` importowane statycznie (`router.ts:16`) | `router.isReady()` przed `mount()`. **XS kod / M weryfikacja** — dotyka ścieżki krytycznej SEO, nie wchodzi bez tripwire + `curl -A Googlebot` + **GSC Live Test** | **WYSOKIE** |
| 5 | **32 usunięte ogłoszenia zwracają HTTP 200 (soft-404).** ✅ **Rozstrzygnięte 2026-07-25: to były ogłoszenia TESTOWE/rozruchowe, usunięte świadomie** — nie przywracać | ID 1–47 (pierwotny seed) nie istnieją w bazie: **411 wyświetleń, 7 klików, pozycje 2,7–8,0**. Spadek PRODUCT_SNIPPETS (157 wyśw./CTR 2,55% → 22 wyśw./0 klików w last28) jest tego skutkiem, ale **strata jest pozorna** — rankowała treść, która nie miała zostać | **410 Gone** zamiast HTTP 200 + szkielet (`noindex` już jest, więc wypadną same, tylko wolniej i kosztem crawl budżetu). `updateOrCreate` **odpada**. Plus 301 dla 2 leafów ze zmienionym slugiem | ~~WYSOKIE~~ **ŚREDNIE** |
| 6 | ~~**5 tras bloga bez seedu**~~ — ✅ **NAPRAWIONE I WDROŻONE 2026-07-25.** Weryfikacja na prodzie po deployu: `seed=1` + `blogList` na **5/5** tras (`/blog`, `/blog/poradniki`, `/blog/lokalizacje`, `/blog/trendy`, `/blog/prawo-i-regulacje`), wszystkie `index, follow`. Prerender: 987 OK / 0 chude / 0 błędy — trasy bloga przeszły przez `needsState` bez retry | **0 z 4 kategorii bloga zindeksowanych.** Potwierdzenie od foundera: GSC Live Test na `/blog/poradniki` → „**Podczas testowania bieżącej wersji wykryto błędy indeksowania**" — mimo że `curl -A Googlebot` daje 200, `index, follow`, 56 485 B i 12 linków do artykułów. Różnica = Google wykonuje JS. Diagnoza: **`seed: 0`** na trasach kategorii | Zamknięte tym samym wzorcem co listingi (`hasLoaded`) i artykuły (`__ssrBlogPost`): (1) `BlogPage.vue` — seed z `__INITIAL_STATE__.blogList`, flaga `hasLoaded` (true tylko po POTWIERDZONYCH danych), `catch` **nie kasuje** zaseedowanej listy, `noindexEmptyCategory` bramkowane na `hasLoaded`; (2) `main.ts` — kolektor zbiera `__ssrBlogList`; (3) `prerender.mjs` — `needsState` obejmuje `/blog` i `/blog/{kategoria}` (regex `^\/blog(\/[^/]+)?$`), `collectState` i `navigateAndCollect` walidują `blogList`. Artykuły świadomie poza `needsState` — mają własny seed i działają. **158/158 testów, `vue-tsc` czysto** | ~~ŚREDNIE~~ ⏳ **czeka na deploy** |
| 7 | **Bajty:** `logo-text.webp` **8056×3303 px / 465 KB** pobierane na KAŻDEJ podstronie, także mobilnej, gdzie CSS je chowa (`AppHeader.vue:1173-1176` — `display:none` nie anuluje `src`); Google Fonts przez `@import` (`style.css:1`) = **791–831 ms** render-blockingu; reCAPTCHA 852 KB + 793–918 ms przy zerowym użyciu przed interakcją; brak `srcset` = 1 755 KiB | Lighthouse ×12 | Przegenerować logo (→ ~800×328), `@import` → `<link>` + `preconnect fonts.gstatic.com`, odroczyć reCAPTCHA do interakcji | ŚREDNIE (XS każde) |
| 8 | **Licznik odsłon zawyżony 13,7×** — `incrementViews` (`AdvertisementController.php:699-719`) liczy prerender (983 trasy/deploy) i crawlery | `stats.php` 591 vs GA4 43 odsłony / 18 userów na tym samym zakresie | Odrzucać HeadlessChrome/boty albo `setRequestInterception` w `prerender.mjs`. **Do czasu naprawy nie prezentować tej liczby wystawcy jako miary zainteresowania** | ŚREDNIE |
| 9 | **6 martwych helperów GA4** — `clickEmail`, `search`, `filterUsed`, `newsletterSubscribe`, `mainContactFormSubmit`, `addToComparison` zdefiniowane w `utils/analytics.ts`, **nigdzie nie wywołane**. Plus `analytics.viewAd` (`AdDetailPage.vue:604-615`) siedzi w bloku zależnym od odpowiedzi API → gubi **37,4% wejść** (115 userów vs 72 zdarzenia) | grep po całym froncie | Wpiąć; `viewAd` przenieść poza blok | ŚREDNIE |
| 10 | ~~**5 padających testów na `master`**~~ — ❌ **ZNALEZISKO BŁĘDNE, wycofane 2026-07-26.** Testy padają **wyłącznie w lokalnym środowisku dev**, nie w repozytorium | **GitHub Actions po pushu `9aba900`: `success` (1m0s)**, tak samo jak poprzedni run 13 dni wcześniej — a backendu nikt w międzyczasie nie tknął (ostatni commit `73ce63b`). Lokalnie: `ManagementTest` ×4 → 422, `SearchAlertTest` → 302. Przyczyna po stronie lokalnego `.env` (`APP_ENV=local`, ustawiony `RECAPTCHA_SECRET_KEY`, `CACHE_STORE` inny niż `array` → `throttle:5,60` na `management/send-link` persystuje między uruchomieniami), nie po stronie kodu. Nie jest to cache configu — `bootstrap/cache/config.php` nie istnieje, `config:clear` nic nie zmienia | **Nic do naprawy w repo.** Jeśli ktoś chce mieć zielone testy lokalnie: uruchamiać z czystym cache i bez sekretu reCAPTCHA, albo dodać `.env.testing`. **Wniosek metodologiczny (drugi raz tego dnia): nie przenosić obserwacji z lokalnego środowiska na wnioski o repo/prodzie bez sprawdzenia CI** — por. korekty w `ANALYTICS_LOG.md` | ~~XS, blokujące~~ **nieaktualne** |

**Drobne:** `<link rel=canonical>` w `spa-fallback.html` · `BingSiteAuth.xml` (weryfikacja Binga wisi na `GSCImport`) · 301 z `www.api.reklamap.pl` (czwarty żywy hostname, 200 bez przekierowania) · `?_v=` → fragment `#` + `X-Robots-Tag` zamiast `Disallow` (blokada **konserwuje** 101 URL-i w indeksie Binga: bot nie pobierze, więc nie zobaczy `noindex`) · `SearchAlertService.php:25-26` ma tę samą klasę błędu co filtr województwa.

### ✅ WDROŻONE 2026-07-25

**DEPLOY WYKONANY 2026-07-25** (`frontend/deploy.sh`): **987 tras prerenderowanych, 0 chudych, 0 błędów**, tripwire 5/5 (`index` + seed + treść + brak fałszywego empty-state).

| Zmiana | Pliki | Status |
|---|---|---|
| **Fix `FAQPage`** — dopasowanie nagłówka po twardym stringu `'Najczęściej zadawane pytania'` gubiło artykuły z `## FAQ`; **3 z 28 nie miały rich snippetu**. Regex + logika wydzielona do testowalnego modułu (wzorzec `listingsSeo.ts`) | `utils/faqSchema.ts` (nowy), `BlogPostPage.vue`, `tests/unit/faqSchema.test.ts` (15 testów) | ✅ **LIVE** — zweryfikowane `curl -A Googlebot`: `citylight-reklama` **7** pytań, `czy-oplaca-sie…` **4**, `jak-zarobic…` **4** (było 0/0/0). 158/158 testów, `vue-tsc` czysto |
| **Microsoft Clarity** — heatmapy/nagrania; bramka na brak `VITE_CLARITY_ID` **oraz na UA bota** (prerender nie zaśmieca nagrań) | `utils/clarity.ts` (nowy), `main.ts`, `.env.example` | ✅ **LIVE** — ID `xs09oppjif` wkompilowane, potwierdzone w produkcyjnym bundlu `/js/index-Brb5vZde.js` |
| **Sitemap drift domknięty deployem** — 4 URL-e serwowane jako `noindex` przez 12 dni | — (skutek deployu) | ✅ **wszystkie 4 = `index, follow`**: `pozwolenie-na-tablice-reklamowa` (63 491 B), `reklama-bez-pozwolenia-kary` (59 125 B), leaf `…jablonowo…-997` (72 190 B, seed OK), leaf `…nowa-wies-elcka…-998` (68 593 B). **Przyczyna systemowa (poz. 2) NADAL otwarta** — check driftu w tripwire niezrobiony |

### ✅ WYKONANE 2026-07-25 poza kodem (GA4 Admin API)

Konto usługi podniesione do roli **Edytujący**, scope `analytics.edit`:
- **Kluczowe zdarzenia:** `contact_phone_click`, `contact_form_submit`, `contact_email_click` — `ONCE_PER_SESSION`. Wcześniej jedynym key eventem był `add_listing_success` → „konwersje" mierzyły wyłącznie podaż.
- **Wymiary niestandardowe (EVENT):** `step_number`, `ad_type`, `ad_city`. Property miała **0 custom dimensions**. Historii nie odzyskuje — zbieranie od 2026-07-25.
- **`dataFilters` nie istnieje w Admin API** (404) → filtr „Internal traffic" rozwiązany u źródła bramką w `index.html` (tag GA4 tylko na `reklamap.pl`/`www.reklamap.pl`). ✅ **WDROŻONE 2026-07-25** — zweryfikowane na prodzie: bezwarunkowy `<script src=googletagmanager>` = **0**, warunek `hostname` obecny w kodzie, measurement ID nadal działa na domenie produkcyjnej. Od teraz dev i prerender nie zanieczyszczają property.

### ✅ ZAMKNIĘTE — obalone pomiarem, NIE implementować

- **Konsolidacja www** — 301 działa, `googleCanonical` = non-www, udział www **33,3% → 6,9% → 2,6%** (ostatnie 7 dni). Agregat 3-miesięczny ciągnął maj–czerwiec. Monitorować, alert dopiero >5%.
- **`preconnect` do `api.reklamap.pl` i kafli OSM** (audyt 07.07 poz. 7-9, 18-21) — **nie pojawiły się ani razu w 12 przebiegach** Lighthouse; prerender + seed zdejmują API z krytycznej ścieżki. Lighthouse wskazał wyłącznie `fonts.gstatic.com` (153–172 ms) i `www.google.com` (95–122 ms).
- **Lazy-LCP** — potwierdzone **tylko dla kategorii na mobile**; leaf ma `lcp-discovery` = 1/1 (jedyny zielony wynik audytu) — **nie ruszać**.
- **„5xx = WAF na `api.reklamap.pl`"** — GSC: `api.reklamap.pl` ma status „**Adres URL jest Google nieznany**". Google nigdy go nie odwiedził. Wszystkie 12 znalezionych 5xx to blog, crawl z 15.05.
- **„Duble miast dzielą podaż"** (audyt 07.07 poz. 11) — realnie **23/827 (2,8%)**. `Ząbkowice` vs `Ząbkowice Śląskie` to różne miasta oddalone o **176 km**, `Duszniki` vs `Duszniki-Zdrój` o 213 km. Sitemapa ma **0 duplikatów `<loc>`**.
- **Meta na CTR-owych trupach** — 78% z nich (30 fraz / 900 wyśw.) to strony poza sitemapą, którym `.htaccess` oddaje szkielet z tytułem strony głównej. **Snippet i tak nie pochodzi z naszej meta.** Meta poprawiać wyłącznie na 4 frazach XS, które mają pod sobą podaż i indeksowalną stronę: `/koszalin`, `/billboardy/dzierzoniow`, `/sosnowiec`, `/reklama-mobilna`.
- **Audyt 07.07 poz. 32 „formularz powinien być `noindex`"** — **podważone**: to landing pozyskania podaży w fazie budowy podaży; `noindex` odciąłby stronę, która ma sprzedawać wystawcom. Zamknąć jako świadomą decyzję.
- **Audyt 07.07 poz. 16 „brak image-sitemap"** — zamknięte, 825 `<image:loc>` (`routes/web.php:200-205`).

### 🟢 NIEZMIENNIKI TRZYMAJĄ (weryfikacja empiryczna, nie ruszać)
218/218 stron = HTTP 200 + `index, follow` · **158/158 canonical = własny URL, 0 rozjazdów** · 60/60 leafów z seedem `__INITIAL_STATE__` · `THIN_PAGE_THRESHOLD=3` spójny w `listingsSeo.ts` / `web.php` / `prerender.mjs` — **0 konfliktów w obie strony** · 301 www→non-www ze ścieżką · bramka UA w `index.html:36`.
**Pokrycie indeksu:** ~299 z 983 URL-i (~30%); kategorie bloga **0/4**, leafy 28%, typy+miasta 39%.

### ⚠️ Znalezisko niedoszacowane w 07.07
**Poz. 4 „near-duplikaty Big Group" jest GORSZA niż opisano:** 516/827 (62,4%) dzieli tytuł, **345 (41,7%) parę tytuł+opis**, trzy URL-e (`…biala-podlaska-{299,302,303}`) są **byte-identyczne** (67 711 / 67 710 / 67 711 B — ten sam `<title>`, meta-description i `<h1>`). Koreluje z 56% leafów w stanie „wykryta, niezindeksowana". `is_verified` = 0 na wszystkich 827. **Import kolejnych 400 billboardów tym samym szablonem pogłębi problem** → warunek dla Biznesowego.
