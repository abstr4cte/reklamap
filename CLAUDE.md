...
- **Luka „między deployami" (ustalenie 2026-09-08).** Prerender jest build-time, więc ogłoszenie lub artykuł opublikowane po ostatnim deployu frontu **nie mają statycznego HTML**: ogłoszenie o poprawnym slugu dostaje z `legacy-ad-redirect.php` surowy `index.html` (bot widzi **tytuł i canonical strony głównej** → Google składa takie strony w jeden klaster duplikatów, patrz Olsztyn 07.2026), a nowy artykuł wpada w `spa-fallback.html` z **`noindex`**. Dlatego **po każdym imporcie nośników i po publikacji artykułu rób deploy frontu**, a przy podejrzeniach porównaj żywą sitemapę (`api.reklamap.pl/sitemap.xml`) ze statyczną (`reklamap.pl/sitemap.xml`) — różnica = strony bez prerenderu. Docelowa naprawa systemowa (fallback PHP z tytułem/canonical z API) opisana w `SEO_TECH_AUDIT.md` 2026-09-08.
...
- **Geo-bucketing / promień aglomeracji — sprawdzić dla WSZYSTKICH top-20 miast przy następnym audycie, nie tylko Katowic — w tym POPYT ("parcie"), nie tylko podaż w promieniu.** Audyt 2026-07-25 (`raport-kod.md` poz. 23, `raport-podaz.md` §6-7) policzył nośniki leżące w promieniu 30 km od miasta bez pokazywania ich na stronie tego miasta: **Katowice 140** (vs 8 pokazywanych), Koszalin 77, Wrocław 38, Poznań 34, Warszawa 13, Radom 13 (+83 w paśmie 30–50 km). Katowice były jedynym miastem dogłębnie zweryfikowanym (140 w zasięgu + potwierdzony popyt 266 wyśw. w GSC) — **Wrocław i Poznań mają realny potencjał podaży, ale nie mają jeszcze sprawdzonego popytu**; przy następnym audycie sprawdzić wyświetlenia GSC dla stron tych miast, żeby ustalić priorytet dalszego rolloutu. Nie zadziała tam, gdzie w promieniu i tak nic nie ma (Białystok: 0 w 50 km, Szczecin: najbliższy nośnik 103 km) — to nie jest rozwiązanie systemowe dla wszystkich białych plam, tylko dla miast w gęstych aglomeracjach.
  **Wdrożenie (pilot Katowice, 2026-08-19):** sekcja "Więcej ofert w okolicy (promień 30 km)" na stronach miast — reużywa istniejący filtr Haversine z `AdvertisementController::buildFilteredQuery` (parametr `include_nearby=1`, zwraca osobne pole `nearby_listings`, NIE miesza się z głównym `data`/`total`). **Świadomy niezmiennik: ta sekcja NIE wpływa na `THIN_PAGE_THRESHOLD`/`noindex`/sitemapę/canonical** — próg cienkiej strony i sygnały indeksacji nadal liczą WYŁĄCZNIE własne ogłoszenia miasta (`web.php` `havingRaw('COUNT(*) >= ?')` na kolumnie `city`, `listingsSeo.ts` `serverTotal`). Pilot celowo ograniczony do miast już ≥3 własne ogłoszenia (już indeksowanych) i tylko stron city-only (nie combo typ×miasto) — rozszerzenie progu cienkiej strony o ogłoszenia z promienia to osobna, ryzykowniejsza decyzja (duplikacja treści między stronami miast), nierozwiązana i poza zakresem tego wdrożenia. Plan: `~/.claude/plans/logical-conjuring-panda.md`.
